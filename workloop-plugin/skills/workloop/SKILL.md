...
For a new requirement, create `.workloop/loops/wl-YYYYMMDD-NN/spec.md` from [spec.md](assets/templates/spec.md), record the current commit as `base_commit`, and use `workloop-spec`.
...
Load [the complete example Loop](references/example-loop/spec.md) only when a filled artifact is needed to resolve ambiguity. Do not load it during routine routing.

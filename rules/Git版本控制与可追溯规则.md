...
1. 在准确的项目根目录执行 `git init`。
2. 检查并补充 `.gitignore`。
3. 排除密钥、凭据、本地配置、构建结果、缓存、依赖目录和大文件。
4. 查看准备纳入基线的文件，禁止直接盲目提交全部文件。
5. 对确认安全的现有项目内容建立初始提交。
6. 将初始提交 SHA 保存为 `baseline_commit`。

没有初始基线提交时，仅执行 `git init` 不算完成，因为此时仍无法可靠恢复原始内容。

如果因为缺少 Git 身份、存在敏感文件或无法判断文件归属而不能创建安全基线，状态进入 `blocked`，不得继续修改项目文件。
...
两个非终态 Loop 或子流程并发修改项目时，单独的 Loop 锁不够。开始前必须：
